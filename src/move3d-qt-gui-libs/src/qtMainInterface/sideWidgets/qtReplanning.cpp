...
ReplanningWidget::ReplanningWidget(QWidget* parent)
    : QWidget(parent), m_ui(new Ui::ReplanningWidget) {
  m_ui->setupUi(this);

  init();
}

ReplanningWidget::~ReplanningWidget() { delete m_ui; }
...
void ReplanningWidget::init() {
  ...
  connect(
      m_ui->pushButtonRunNoReset, SIGNAL(clicked()), this, SLOT(runNoReset()));

  connect(m_ui->pushButtonExecuteSimpleSim,
          SIGNAL(clicked()),
          this,
          SLOT(executeSimpleSimu()));

  // Enable multi-thread graphical mode
  connect(m_ui->checkBoxMultiThreadGraphical,
          SIGNAL(toggled(bool)),
          this,
          SLOT(multiThreadGraphicalMode(bool)));

  // Active Joints
  connect(m_ui->radioButtonNavigation,
          SIGNAL(toggled(bool)),
          this,
          SLOT(setActiveJoints()));
  connect(m_ui->radioButtonManipulation,
          SIGNAL(toggled(bool)),
          this,
          SLOT(setActiveJoints()));
  connect(m_ui->radioButtonMobileManip,
          SIGNAL(toggled(bool)),
          this,
          SLOT(setActiveJoints()));
  connect(ENV.getObject(Env::setOfActiveJoints),
          SIGNAL(valueChanged(int)),
          this,
          SLOT(setActiveJointsRadioButtons(int)),
          Qt::DirectConnection);

  // Init Method
  connect(
      m_ui->radioButtonRRT, SIGNAL(toggled(bool)), this, SLOT(setInitMethod()));
  connect(m_ui->radioButtonStraightLine,
          SIGNAL(toggled(bool)),
          this,
          SLOT(setInitMethod()));
  connect(PlanEnv->getObject(PlanParam::replanningInitMethod),
          SIGNAL(valueChanged(int)),
          this,
          SLOT(setInitMethodRadioButtons(int)),
          Qt::DirectConnection);

  // replanner Type
  connect(m_ui->comboBoxReplanner,
          SIGNAL(currentIndexChanged(int)),
          PlanEnv->getObject(PlanParam::replanningAlgorithm),
          SLOT(set(int)));
  connect(PlanEnv->getObject(PlanParam::replanningAlgorithm),
          SIGNAL(valueChanged(int)),
          m_ui->comboBoxReplanner,
...
// Plot Noisy trajectories
#ifdef USE_QWT
  connect(m_ui->pushButtonPlotNoise,
          SIGNAL(clicked()),
          this,
          SLOT(plotNoisyTrajectories()));
  connect(m_ui->pushButtonPlotTraj,
          SIGNAL(clicked()),
          this,
          SLOT(plotMultiVectors()));
  connect(m_ui->pushButtonPlotConvergence,
          SIGNAL(clicked()),
          this,
          SLOT(plotConvergence()));
  m_plot = new BasicPlotWindow();
#endif

  connect(m_ui->pushButtonComputeSM,
          SIGNAL(clicked()),
          this,
          SLOT(computeSoftMotion()));
  connect(m_ui->pushButtonFixJoints,
          SIGNAL(clicked()),
          this,
          SLOT(setMlpCntrtsAndFixJoints()));

  // Smooth And Obstacle Weigth
  new SpinBoxConnector(this,
                       m_ui->doubleSpinBoxSmoothWeight,
                       PlanEnv->getObject(PlanParam::trajOptimSmoothWeight));
  new SpinBoxConnector(this,
                       m_ui->doubleSpinBoxObstacWeight,
                       PlanEnv->getObject(PlanParam::trajOptimObstacWeight));
  new SpinBoxConnector(this,
  ...
  new SpinBoxConnector(this,
                       m_ui->doubleSpinBoxSmoothFactor,
                       PlanEnv->getObject(PlanParam::trajOptimSmoothFactor));

  // Smooth Weights
  new SpinBoxConnector(this,
                       m_ui->doubleSpinBoxSmoothVel,
                       PlanEnv->getObject(PlanParam::trajStompSmoothVel));
  new SpinBoxConnector(this,
                       m_ui->doubleSpinBoxSmoothAcc,
                       PlanEnv->getObject(PlanParam::trajStompSmoothAcc));
  new SpinBoxConnector(this,
                       m_ui->doubleSpinBoxSmoothJerk,
                       PlanEnv->getObject(PlanParam::trajStompSmoothJerk));

  //---------------------------------------
  // Test the multi gaussian
  new connectCheckBoxToEnv(m_ui->checkBoxActiveJointsSetAtStart,
                           ENV.getObject(Env::setActiveJointsGroup));
  new connectCheckBoxToEnv(m_ui->checkBoxInitStomp,
                           ENV.getObject(Env::setStompPlanner));

  new connectCheckBoxToEnv(
      m_ui->checkBoxTestMultiGauss,
      PlanEnv->getObject(PlanParam::trajOptimTestMultiGauss));
  new connectCheckBoxToEnv(m_ui->checkBoxDrawTraj,
                           ENV.getObject(Env::drawTraj));
  new connectCheckBoxToEnv(m_ui->checkBoxOptimizeCurrentTraj,
                           PlanEnv->getObject(PlanParam::withCurrentTraj));
  new connectCheckBoxToEnv(m_ui->checkBoxDoReplanning,
                           PlanEnv->getObject(PlanParam::doReplanning));
  new connectCheckBoxToEnv(m_ui->checkBoxMoveHuman,
                           PlanEnv->getObject(PlanParam::trajMoveHuman));
  new connectCheckBoxToEnv(m_ui->checkBoxRecomputeOtp,
                           PlanEnv->getObject(PlanParam::trajUseOtp));
  new connectCheckBoxToEnv(m_ui->checkBoxSelectedDuration,
                           PlanEnv->getObject(PlanParam::useSelectedDuration));
  new connectCheckBoxToEnv(
      m_ui->checkBoxTimeLimit,
      PlanEnv->getObject(PlanParam::trajStompWithTimeLimit));
  new connectCheckBoxToEnv(m_ui->checkBoxMMatrix,
                           PlanEnv->getObject(PlanParam::trajStompMultiplyM));
  new connectCheckBoxToEnv(m_ui->checkBoxWithRRT,
                           PlanEnv->getObject(PlanParam::trajStompWithRRT));
  new connectCheckBoxToEnv(
      m_ui->checkBoxCovAdaptation,
      PlanEnv->getObject(PlanParam::trajStompMatrixAdaptation));
  new connectCheckBoxToEnv(
      m_ui->checkBoxStompMaxIteration,
      PlanEnv->getObject(PlanParam::trajStompWithIterLimit));
  new connectCheckBoxToEnv(m_ui->checkBoxDrawParallel,
                           PlanEnv->getObject(PlanParam::drawParallelTraj));
  //    new connectCheckBoxToEnv( m_ui->checkBoxDrawInCollision,
  //    PlanEnv->getObject(PlanParam::drawScaleFactorNodeSphere) );
  new connectCheckBoxToEnv(m_ui->checkBoxStompNoPrint,
                           PlanEnv->getObject(PlanParam::trajStompNoPrint));
  new connectCheckBoxToEnv(m_ui->checkBoxMultipleStomp,
                           PlanEnv->getObject(PlanParam::trajStompRunMultiple));
  new connectCheckBoxToEnv(m_ui->checkBoxStompParallel,
                           PlanEnv->getObject(PlanParam::trajStompRunParallel));
  new connectCheckBoxToEnv(
      m_ui->checkBoxStompDrawInCollision,
      PlanEnv->getObject(PlanParam::trajStompDrawImprovement));
  new connectCheckBoxToEnv(
      m_ui->checkBoxMoveEndConfig,
      PlanEnv->getObject(PlanParam::trajStompMoveEndConfig));
  new connectCheckBoxToEnv(
      m_ui->checkBoxStopWhenCollisionFree,
      PlanEnv->getObject(PlanParam::trajOptimStopWhenCollisionFree));

  // Stomp max iteration
  new SpinBoxConnector(this,
                       m_ui->spinBoxStompMaxIteration,
                       PlanEnv->getObject(PlanParam::stompMaxIteration));

  // Stomp draw iteration
  new SpinBoxConnector(this,
                       m_ui->spinBoxStompDrawIteration,
                       PlanEnv->getObject(PlanParam::stompDrawIteration));

  // Time Limit
  new SpinBoxConnector(this,
                       m_ui->doubleSpinBoxTimeLimit,
                       PlanEnv->getObject(PlanParam::trajStompTimeLimit));

  // Set the number of point to be optimized
  new SpinBoxConnector(this,
                       m_ui->spinBoxNbPoints,
                       PlanEnv->getObject(PlanParam::nb_pointsOnTraj));
  ...
  new SpinBoxSliderConnector(
      this,
      m_ui->doubleSpinBoxReplanningWindow,
      m_ui->horizontalSliderReplanningWindow,
      PlanEnv->getObject(PlanParam::trajReplanningWindow));
  new SpinBoxSliderConnector(
      this,
      m_ui->doubleSpinBoxTotalTrajDuration,
      m_ui->horizontalSliderTotalTrajDuration,
      PlanEnv->getObject(PlanParam::trajReplanningTotalTime));
}
...
void ReplanningWidget::plotNoisyTrajectories() {
  ...
  //	{
  //    cout << "global_noiseTrajectory[" << i << "] = " <<
  //    global_noiseTrajectory[i] << endl;
  //  }
...
#if defined(USE_QWT_5)
  if (traj_optim_convergence.size() != 0 &&
      traj_optim_convergence[0].size() != 0) {
    MultiPlot* myPlot = new MultiPlot(m_plot);
    myPlot->setGeometry(m_plot->getPlot()->geometry());
