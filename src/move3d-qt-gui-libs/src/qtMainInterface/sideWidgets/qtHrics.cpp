...
HricsWidget::HricsWidget(QWidget* parent)
    : QWidget(parent), m_ui(new Ui::HricsWidget) {
  m_ui->setupUi(this);
...
void HricsWidget::initHRI() {
  ...
  new connectCheckBoxToEnv(m_ui->checkBoxDrawGrid,
                           ENV.getObject(Env::drawGrid));
  new connectCheckBoxToEnv(m_ui->checkBoxEntireGrid,
                           ENV.getObject(Env::drawEntireGrid));
  new connectCheckBoxToEnv(m_ui->checkBoxDrawDistance,
                           ENV.getObject(Env::drawDistance));
  new connectCheckBoxToEnv(m_ui->checkBoxDrawRandPoints,
                           ENV.getObject(Env::drawPoints));
  new connectCheckBoxToEnv(m_ui->checkBoxDrawGaze,
                           ENV.getObject(Env::drawGaze));
  new connectCheckBoxToEnv(m_ui->checkBoxDrawVectorField,
                           ENV.getObject(Env::drawVectorField));
  new connectCheckBoxToEnv(m_ui->checkBoxDrawOnlyOneLine,
                           ENV.getObject(Env::drawOnlyOneLine));
  new connectCheckBoxToEnv(m_ui->checkBoxDrawBox, ENV.getObject(Env::drawBox));
  new connectCheckBoxToEnv(m_ui->checkBoxDrawReachable,
                           PlanEnv->getObject(PlanParam::drawReachableGrid));
  new connectCheckBoxToEnv(m_ui->checkBoxDrawHumanColorFromConf,
                           PlanEnv->getObject(PlanParam::drawNaturalColor));

  new connectCheckBoxToEnv(m_ui->checkBoxHRICS_MOPL,
                           ENV.getObject(Env::HRIPlannerWS));
  //	new connectCheckBoxToEnv(m_ui->checkBoxBBDist,
  //Env::useBoxDist);
  //	new connectCheckBoxToEnv(m_ui->checkBoxBallDist,
  //Env::useBallDist);
  new connectCheckBoxToEnv(m_ui->checkBoxHriPlannerTRRT,
                           ENV.getObject(Env::HRIPlannerTRRT));
  new connectCheckBoxToEnv(m_ui->checkBoxHriPathDistance,
                           ENV.getObject(Env::HRIPathDistance));
  new connectCheckBoxToEnv(m_ui->checkBoxCameraBehindHuman,
                           ENV.getObject(Env::HRIcameraBehindHuman));
  new connectCheckBoxToEnv(m_ui->checkBoxHRINoRobot,
                           ENV.getObject(Env::HRINoRobot));
  new connectCheckBoxToEnv(m_ui->checkBoxAutoLoadGrid,
                           ENV.getObject(Env::HRIAutoLoadGrid));

  connect(m_ui->checkBoxDrawGrid,
          SIGNAL(clicked()),
          m_mainWindow,
          SLOT(drawAllWinActive()));
  connect(m_ui->checkBoxEntireGrid,
          SIGNAL(clicked()),
          m_mainWindow,
          SLOT(drawAllWinActive()));

  connect(m_ui->pushButtonInitHRICS,
          SIGNAL(clicked()),
          this,
          SLOT(make3DHriGrid()));

  connect(
      m_ui->pushButtonInitGrids, SIGNAL(clicked()), this, SLOT(initGrids()));
  connect(m_ui->pushButtonDeleteGrids,
          SIGNAL(clicked()),
          this,
          SLOT(deleteGrids()));
  connect(m_ui->pushButtonComputeAllCellCost,
          SIGNAL(clicked()),
          this,
          SLOT(computeAllCellCost()));
  ...
  // Simple Otp test
  connect(m_ui->pushButtonComputeOtpConfig,
          SIGNAL(clicked()),
          this,
          SLOT(computeOtpConfig()));

  // A*
  connect(m_ui->pushButtonAStarInGrid,
          SIGNAL(clicked()),
          this,
          SLOT(computeAStarGrid()));
  ...
  // Wich Test
  // -------------------------------
  connect(m_ui->whichTestBox,
          SIGNAL(currentIndexChanged(int)),
          ENV.getObject(Env::hriCostType),
          SLOT(set(int)),
          Qt::DirectConnection);
  connect(ENV.getObject(Env::hriCostType),
          SIGNAL(valueChanged(int)),
          this,
          SLOT(setWhichTestSlot(int)),
          Qt::DirectConnection);
...
void HricsWidget::drawAllWinActive() { m_mainWindow->drawAllWinActive(); }
...
void HricsWidget::initVectorField() {
  connect(m_ui->pushButtonVectorField,
          SIGNAL(clicked()),
          this,
          SLOT(computeVectorField()));
}
...
void HricsWidget::initObjectTransferPoint() {
  connect(m_k_distance,
          SIGNAL(valueChanged(double)),
          this,
          SLOT(computeObjectTransferPoint()));
  connect(m_k_visbility,
          SIGNAL(valueChanged(double)),
          this,
          SLOT(computeObjectTransferPoint()));
  connect(m_k_reachability,
          SIGNAL(valueChanged(double)),
          this,
          SLOT(computeObjectTransferPoint()));
}
...
void HricsWidget::computeObjectTransferPoint() {
  ...
  if (ENV.getBool(Env::HRIComputeOTP)) {
    ...
    if (dynamic_cast<HRICS::Workspace*>(HRICS_MotionPL)
            ->computeBestTransferPoint(WSPoint)) {
      ...
      Robot* Object =
          global_Project->getActiveScene()->getRobotByNameContaining("OBJECT");

      confPtr_t q_curr = Object->getCurrentPos();
      (*q_curr)[6] = WSPoint[0];
      (*q_curr)[7] = WSPoint[1];
      (*q_curr)[8] = WSPoint[2];
      // qDebug() << "| z : "<< WSPoint[0] <<"| z : "<<  WSPoint[1] <<  "| z :
      // "<< WSPoint[2] ;
...
void HricsWidget::initDrawOneLineInGrid() {
  connect(m_ui->radioButtonXline,
          SIGNAL(toggled(bool)),
          this,
          SLOT(set_X_line(bool)));
  connect(m_ui->radioButtonYline,
          SIGNAL(toggled(bool)),
          this,
          SLOT(set_Y_line(bool)));
  connect(m_ui->radioButtonZline,
          SIGNAL(toggled(bool)),
          this,
          SLOT(set_Z_line(bool)));

  QtShiva::SpinBoxSliderConnector* connector =
      new QtShiva::SpinBoxSliderConnector(this,
                                          m_ui->doubleSpinBoxWhichGridLine,
                                          m_ui->horizontalSliderWhichGridLine,
                                          ENV.getObject(Env::hriShownGridLine));
...
void HricsWidget::set_X_line(bool enable) {
  if (enable && API_activeGrid) {
    unsigned int maxLines =
        dynamic_cast<Move3D::ThreeDGrid*>(API_activeGrid)->getXNumberOfCells();
...
void HricsWidget::set_Y_line(bool enable) {
  if (enable && API_activeGrid) {
    unsigned int maxLines =
        dynamic_cast<Move3D::ThreeDGrid*>(API_activeGrid)->getYNumberOfCells();
    m_ui->doubleSpinBoxWhichGridLine->setMaximum(maxLines);
  }
...
void HricsWidget::set_Z_line(bool enable) {
  if (enable && API_activeGrid) {
    unsigned int maxLines =
        dynamic_cast<Move3D::ThreeDGrid*>(API_activeGrid)->getZNumberOfCells();
    m_ui->doubleSpinBoxWhichGridLine->setMaximum(maxLines);
  }
...
void HricsWidget::initGrids() {
  ...
  if (HRICS_humanCostMaps != NULL) delete HRICS_humanCostMaps;

  // Build a costspace from the Robot and Humans in the workspace
  // This costspace will hold the agents grids
  HRICS_humanCostMaps = new HRICS::HumanCostSpace(ws->getRobot(),
                                                  ws->getHumans(),
                                                  HRICS_activeNatu,
