...
CostWidget::CostWidget(QWidget* parent)
    : QWidget(parent), m_ui(new Ui::CostWidget) {
  m_ui->setupUi(this);
  ...
  m_tabTrajectorySampling =
      new TrajectorySamplingWidget(m_ui->TrajectorySampling);
...
#ifdef HRI_COSTSPACE
HricsWidget* CostWidget::getHriWidget() { return m_tabHri; }

OtpWidget* CostWidget::getOtpWidget() { return m_tabOtp; }

HriGestureWidget* CostWidget::getGestureWidget() { return m_tabGesture; }
#endif

#if defined(LIGHT_PLANNER) && defined(MULTILOCALPATH)
ReplanningWidget* CostWidget::getReplanningWidget() { return m_tabReplan; }
#endif

DistFieldWidget* CostWidget::getDistFieldWidget() { return m_ui->tabDistField; }
...
void CostWidget::initCost() {
  ...
  new connectCheckBoxToEnv(m_ui->isCostSpaceCopy,
                           ENV.getObject(Env::isCostSpace));
  connect(m_ui->isCostSpaceCopy,
          SIGNAL(toggled(bool)),
          SLOT(envIsCostSpaceValueChanged(bool)));
  new connectCheckBoxToEnv(m_ui->checkBoxUseTRRT, ENV.getObject(Env::useTRRT));
  connect(m_ui->checkBoxUseTRRT,
          SIGNAL(toggled(bool)),
          SLOT(envUseTRRTValueChanged(bool)));
  new connectCheckBoxToEnv(m_ui->checkBoxCostBefore,
                           ENV.getObject(Env::costBeforeColl));
  new connectCheckBoxToEnv(m_ui->checkBoxCostExpandToGoal,
                           ENV.getObject(Env::costExpandToGoal));
  new connectCheckBoxToEnv(m_ui->checkBoxCostWithGradient,
                           ENV.getObject(Env::tRrtComputeGradient));
  new connectCheckBoxToEnv(
      m_ui->checkBoxPrintAndComputeCostAfterPlannif,
      PlanEnv->getObject(PlanParam::trajComputeCostAfterPlannif));

  connect(m_ui->pushButtonInitCostSpace,
          SIGNAL(clicked()),
          this,
          SLOT(initCostSpace()));

  new SpinBoxSliderConnector(this,
                             m_ui->doubleSpinBoxInitTemp,
                             m_ui->horizontalSliderInitTemp,
                             ENV.getObject(Env::initialTemperature));
  new SpinBoxSliderConnector(this,
                             m_ui->doubleSpinBoxTempRate,
                             m_ui->horizontalSliderTempRate,
                             ENV.getObject(Env::temperatureRate));
  new SpinBoxSliderConnector(this,
                             m_ui->doubleSpinBoxLengthWeight,
                             m_ui->horizontalSliderLengthWeight,
                             ENV.getObject(Env::KlengthWeight));
  new SpinBoxSliderConnector(this,
                             m_ui->doubleSpinBoxResolution,
                             m_ui->horizontalSliderResolution,
                             PlanEnv->getObject(PlanParam::costResolution));
  new SpinBoxSliderConnector(this,
                             m_ui->doubleSpinBoxMinConnectGap,
                             m_ui->horizontalSliderMinConnectGap,
                             ENV.getObject(Env::minimalFinalExpansionGap));
  new SpinBoxSliderConnector(this,
                             m_ui->doubleSpinBoxMaxCost,
                             m_ui->horizontalSliderMaxCost,
                             ENV.getObject(Env::costMax));

#ifdef USE_QWT
  connect(m_ui->pushButtonShowTrajCost,
          SIGNAL(clicked()),
          this,
          SLOT(showTrajCost()));
  connect(m_ui->pushButtonShowCostProfile,
          SIGNAL(clicked()),
          this,
          SLOT(showCostProfile()));
  connect(m_ui->pushButtonShowHRITrajCost,
          SIGNAL(clicked()),
          this,
          SLOT(showHRITrajCost()));
  connect(m_ui->pushButtonShowSTOMPCost,
          SIGNAL(clicked()),
          this,
          SLOT(showSTOMPTrajCost()));
  connect(m_ui->pushButtonShowTemp,
          SIGNAL(clicked()),
          this,
          SLOT(showTemperature()));
  ...
  connect(ENV.getObject(Env::costAlongTraj),
          SIGNAL(valueChanged(std::vector<double>)),
          this,
          SLOT(setPlotedVector(std::vector<double>)));
  //    connect(m_ui->pushButtonShowTrajCost,SIGNAL(clicked()),this->plot,SLOT(show()));
  connect(m_ui->pushButtonGridInGraph,
          SIGNAL(clicked()),
          this,
          SLOT(putGridInGraph()));
  ...
  // costCriterium
  connect(m_ui->comboBoxTrajCostEstimation,
          SIGNAL(currentIndexChanged(int)),
          this,
          SLOT(setCostCriterium(int)));
  m_ui->comboBoxTrajCostEstimation->setCurrentIndex(0);

  connect(m_ui->comboBoxDistanceEstimation,
          SIGNAL(currentIndexChanged(int)),
          this,
          SLOT(setDistanceCriterium(int)));
  ...
  connect(m_ui->pushButton2DDijkstra,
          SIGNAL(clicked()),
          this,
          SLOT(graphSearchTest()));

  connect(m_ui->pushButtonRecomputeGraph,
          SIGNAL(clicked()),
          this,
          SLOT(newGraphAndReComputeCost()));
  connect(m_ui->pushButtonExtractBestPath,
          SIGNAL(clicked()),
          this,
          SLOT(extractBestPath()));
  connect(m_ui->pushButtonStonesGraph,
          SIGNAL(clicked()),
          this,
          SLOT(stonesGraph()));
}
...
void CostWidget::setCostFunction(std::string function) {
  ...
  vector<string>::iterator it =
      std::find(costFunc.begin(), costFunc.end(), function);
  if (it == costFunc.end()) {
...
void CostWidget::setCostFunction(int costFunctionId) {
  ...
  if (costFunctionId < int(AllCost.size()) && costFunctionId >= 0) {
    PlanEnv->setString(PlanParam::active_cost_function,
                       AllCost[costFunctionId]);
    global_costSpace->setCost(AllCost[costFunctionId]);
    cout << "Cost Function is now :  "
         << PlanEnv->getString(PlanParam::active_cost_function) << endl;
  }
}
...
void CostWidget::setDistanceCriterium(int distance) {
  if (global_costSpace == NULL) {
    ...
    switch (distance) {
      case 0:
        global_costSpace->setDistanceMethod(cs_classic);
        break;
      case 1:
        global_costSpace->setDistanceMethod(cs_pr2_manip);
        break;
    }
  }
}
...
void CostWidget::stonesGraph() {
  ...
  const char* file = std::string(
                         "/Users/jmainpri/workspace/BioMove3DDemos/PathDeform/"
                         "simple/StonesACR.graph").c_str();
...
void CostWidget::showTrajCost() {
  ...
  if (traj.size() == 0) {
    ...
    cout << "Cost for ( nb points : " << nb_points << " ) : " << cost_n_points
         << endl;
...
void CostWidget::showHRITrajCost() {
  ...
  oss << getenv("HOME_MOVE3D") << "/statFiles/CostAlongTraj_" << std::setw(3)
      << std::setfill('0') << count_hri_traj++ << ".csv";
  ...
  vector<double> costDistance, costVisibility, costReachable, costCombine,
      costGrouping, costTotal;
...
void CostWidget::showSTOMPTrajCost() {
  ...
  if (global_optimizer) {
    ...
    global_optimizer->getCostProfiles(
        smoothness_cost, collision_cost, general_cost);
  ...
  for (int j = 0; j < nb_sample; j++) {
    ...
    if (floor(k) >= smoothness_cost.size() ||
        floor(k) >= collision_cost.size() || floor(k) >= general_cost.size())
      break;
  }
