...
DistFieldWidget::DistFieldWidget(QWidget* parent)
    : QWidget(parent), m_ui(new Ui::DistFieldWidget) {
  m_ui->setupUi(this);
}

DistFieldWidget::~DistFieldWidget() { delete m_ui; }

void DistFieldWidget::initDistField() {
  // This function enables multi-threading
  //  connect(this, SIGNAL(selectedPlanner(QString)), global_plannerHandler,
  //  SLOT(startPlanner(QString)));
  connect(m_ui->pushButtonCreateDistanceField,
          SIGNAL(clicked()),
          this,
          SLOT(createDistanceField()));
  connect(m_ui->pushButtonDeleteDistanceField,
          SIGNAL(clicked()),
          this,
          SLOT(deleteDistanceField()));
  connect(m_ui->pushButtonAddAllPoints,
          SIGNAL(clicked()),
          this,
          SLOT(addAllPointsToField()));
  connect(m_ui->pushButtonGenerateRobotBoundingVolumes,
          SIGNAL(clicked()),
          this,
          SLOT(generateRobotBoundingVolumes()));

  new connectCheckBoxToEnv(m_ui->checkBoxDrawOccupVoxels,
                           PlanEnv->getObject(PlanParam::drawOccupVoxels));
  new connectCheckBoxToEnv(m_ui->checkBoxDrawSampledPoints,
                           PlanEnv->getObject(PlanParam::drawSampledPoints));
  new connectCheckBoxToEnv(m_ui->checkBoxDrawStaticCells,
                           PlanEnv->getObject(PlanParam::drawStaticVoxels));
  new connectCheckBoxToEnv(m_ui->checkBoxDrawBoundingVolumes,
                           PlanEnv->getObject(PlanParam::drawBoundingVolumes));

  connect(m_ui->checkBoxDrawOccupVoxels,
          SIGNAL(toggled(bool)),
          m_mainWindow,
          SLOT(drawAllWinActive()),
          Qt::QueuedConnection);
  connect(m_ui->checkBoxDrawSampledPoints,
          SIGNAL(toggled(bool)),
          m_mainWindow,
          SLOT(drawAllWinActive()),
          Qt::QueuedConnection);
  connect(m_ui->checkBoxDrawStaticCells,
          SIGNAL(toggled(bool)),
          m_mainWindow,
          SLOT(drawAllWinActive()),
          Qt::QueuedConnection);
  connect(m_ui->checkBoxDrawBoundingVolumes,
          SIGNAL(toggled(bool)),
          m_mainWindow,
          SLOT(drawAllWinActive()),
          Qt::QueuedConnection);
  ...
  SpinBoxConnector(this,
                   m_ui->doubleSpinBoxDrawingDistance,
                   PlanEnv->getObject(PlanParam::distMinToDraw));
  ...
  SpinBoxConnector(this,
                   m_ui->doubleSpinBoxRatioRadiusSpacing,
                   PlanEnv->getObject(PlanParam::ratioCollRadiusSpacing));

  SpinBoxConnector(this,
                   m_ui->doubleSpinBoxCollisionPointsClearance,
                   PlanEnv->getObject(PlanParam::collison_points_clearance));
}
...
void DistFieldWidget::createDistanceField() {
  ...
  global_collisionSpace =
      new CollisionSpace(sce->getActiveRobot(),
...
void DistFieldWidget::generateRobotBoundingVolumes() {
  ...
  if (!planner_joints.empty()) {
    ...
    sampler->generateRobotCollisionPoints(
        rob, active_joints, planner_joints_id);
