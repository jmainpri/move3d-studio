...
HriGestureWidget::HriGestureWidget(QWidget* parent)
    : QWidget(parent), m_ui(new Ui::HriGestureWidget) {
  m_ui->setupUi(this);
}
...
void HriGestureWidget::initRecordedMotion() {
  ...
  new connectCheckBoxToEnv(m_ui->checkBoxInitGestureModule,
                           GestEnv->getObject(GestParam::init_module_at_start));
  new connectCheckBoxToEnv(
      m_ui->checkBoxPlaySimulationMotions,
      HriEnv->getObject(HricsParam::ioc_show_last_simulation));

  connect(m_ui->pushButtonLoadRecordedMotion,
          SIGNAL(clicked()),
          this,
          SLOT(loadRecordedMotion()));
  connect(m_ui->pushButtonShowRecordedMotion,
          SIGNAL(clicked()),
          this,
          SLOT(showRecordedMotion()));

  connect(m_ui->spinBoxShowConfigurationRecordedMotion,
          SIGNAL(valueChanged(int)),
          this,
          SLOT(getIthConfigurationInMotion()),
          Qt::QueuedConnection);
  connect(
      m_ui->pushButtonSetBeginRM, SIGNAL(clicked()), this, SLOT(setSourceRM()));
  connect(
      m_ui->pushButtonSetEndRM, SIGNAL(clicked()), this, SLOT(setTargetRM()));
  connect(m_ui->pushButtonExtarctAndSaveRM,
          SIGNAL(clicked()),
          this,
          SLOT(extractAndSaveRM()));
  connect(
      m_ui->pushButtonLoadFolder, SIGNAL(clicked()), this, SLOT(loadFolder()));
  connect(m_ui->pushButtonSaveToCSV,
          SIGNAL(clicked()),
          this,
          SLOT(convertFolderToCSV()));
  connect(m_ui->pushButtonLoadFromCSV,
          SIGNAL(clicked()),
          this,
          SLOT(loadFromCSV()));

  connect(m_ui->pushButtonLoadTwoFolders,
          SIGNAL(clicked()),
          this,
          SLOT(loadFolderTwoHumans()));
  connect(m_ui->pushButtonSelectCurrentMotion,
          SIGNAL(clicked()),
          this,
          SLOT(selectCurrentMotion()));
...
void HriGestureWidget::loadRecordedMotion() {
  ...
  global_motionRecorders[0]->loadMotionFromMultipleFiles(
      "/home/jmainpri/workspace/move3d/libmove3d/statFiles/recorded_motion/"
      "motion_saved_",
      37);
...
void HriGestureWidget::extractAndSaveRM() {
  if (!global_motionRecorders.empty()) {
    ...
    filename << std::setw(5) << std::setfill('0') << m_saved_file_id++
             << ".xml";
...
// "/home/jmainpri/workspace/move3d/data/Library_Untouched/Sorted/8";
std::string foldername =
    "/home/jmainpri/workspace/move3d/libmove3d/statFiles/collaboration/"
    "recorded_motion_01_09_13";
...
void HriGestureWidget::loadFolder() {
  if (global_motionRecorders.empty()) {
    ...
    Robot* human =
        global_Project->getActiveScene()->getRobotByName("HERAKLES_HUMAN1");
    if (human == NULL) {
...
void HriGestureWidget::initWorkspaceOccupancy() {
  connect(m_ui->pushButtonSetMotionsAndComputeOccupancy,
          SIGNAL(clicked()),
          this,
          SLOT(computeWorkspaceOccupancy()));
  connect(m_ui->pushButtonClassifyMotion,
          SIGNAL(clicked()),
          this,
          SLOT(classifyMotion()));
  connect(m_ui->spinBoxClassToDraw,
          SIGNAL(valueChanged(int)),
          this,
          SLOT(setClassToDraw(int)));

  new connectCheckBoxToEnv(m_ui->checkBoxPlayNext,
                           GestEnv->getObject(GestParam::play_next));
  new connectCheckBoxToEnv(m_ui->checkBoxRepeat,
                           GestEnv->getObject(GestParam::play_repeat));

  new connectCheckBoxToEnv(
      m_ui->checkBoxDrawHumanSampledPoints,
      GestEnv->getObject(GestParam::draw_human_sampled_points));
  new connectCheckBoxToEnv(
      m_ui->checkBoxDrawRobotSampledPoints,
      GestEnv->getObject(GestParam::draw_robot_sampled_points));
  new connectCheckBoxToEnv(m_ui->checkBoxDrawVoxelOccupancy,
                           GestEnv->getObject(GestParam::draw_ws_occupancy));
  new connectCheckBoxToEnv(m_ui->checkBoxDrawOneClassOnly,
                           GestEnv->getObject(GestParam::draw_single_class));
  new connectCheckBoxToEnv(m_ui->checkBoxDrawHumanTrajectory,
                           GestEnv->getObject(GestParam::draw_recorded_motion));
  new connectCheckBoxToEnv(
      m_ui->checkBoxDrawCurrentOccupancy,
      GestEnv->getObject(GestParam::draw_current_occupancy));

  new connectCheckBoxToEnv(m_ui->checkBoxMultipleStomps,
                           GestEnv->getObject(GestParam::with_multiple_stomps));
  new connectCheckBoxToEnv(m_ui->checkBoxParallelize,
                           GestEnv->getObject(GestParam::parallelize_stomp));

  connect(m_ui->pushButtonStartGestureSimulation,
          SIGNAL(clicked()),
          this,
          SLOT(startGestureSimulation()));
}

void HriGestureWidget::computeWorkspaceOccupancy() {
  if (global_workspaceOccupancy == NULL || global_motionRecorders.empty()) {
    cout << "global_workspaceOccupancy or global_motionRecorder are not "
            "initilized" << endl;
    return;
  }
...
void HriGestureWidget::initHriIOC() {
  new connectCheckBoxToEnv(m_ui->checkBoxInitHriIOC,
                           GestEnv->getObject(GestParam::init_module_ioc));
  new connectCheckBoxToEnv(m_ui->checkBoxInitSphereCost,
                           HriEnv->getObject(HricsParam::init_spheres_cost));
  new connectCheckBoxToEnv(
      m_ui->checkBoxInitHumanTrajCost,
      HriEnv->getObject(HricsParam::init_human_trajectory_cost));
  new connectCheckBoxToEnv(m_ui->checkBoxIocExitAfterRun,
                           HriEnv->getObject(HricsParam::ioc_exit_after_run));

  new connectCheckBoxToEnv(m_ui->checkBoxGestureDebug,
                           GestEnv->getObject(GestParam::print_debug));
  new connectCheckBoxToEnv(m_ui->checkBoxSingleIteration,
                           HriEnv->getObject(HricsParam::ioc_single_iteration));
  new connectCheckBoxToEnv(m_ui->checkBoxDrawOneDemo,
                           HriEnv->getObject(HricsParam::ioc_draw_one_demo));
  new connectCheckBoxToEnv(m_ui->checkBoxRemoveSplit,
                           HriEnv->getObject(HricsParam::ioc_remove_split));

  new connectCheckBoxToEnv(
      m_ui->checkBoxDrawDemonstrations,
      HriEnv->getObject(HricsParam::ioc_draw_demonstrations));
  new connectCheckBoxToEnv(m_ui->checkBoxDrawSamples,
                           HriEnv->getObject(HricsParam::ioc_draw_samples));
  new connectCheckBoxToEnv(
      m_ui->checkBoxLoadSampleFromFile,
      HriEnv->getObject(HricsParam::ioc_load_samples_from_file));
  new connectCheckBoxToEnv(
      m_ui->checkBoxSampleAroundDemo,
      HriEnv->getObject(HricsParam::ioc_sample_around_demo));
  new connectCheckBoxToEnv(
      m_ui->checkBoxUseSimulationDemos,
      HriEnv->getObject(HricsParam::ioc_use_simulation_demos));
  new connectCheckBoxToEnv(
      m_ui->checkBoxUserSetPelvisBounds,
      HriEnv->getObject(HricsParam::ioc_user_set_pelvis_bounds));

  new connectCheckBoxToEnv(
      m_ui->checkBoxIOCSetDofLimits,
      HriEnv->getObject(HricsParam::ioc_set_dof_limits_from_experiments));

  new connectCheckBoxToEnv(m_ui->checkBoxNoReplanning,
                           HriEnv->getObject(HricsParam::ioc_no_replanning));
  new connectCheckBoxToEnv(m_ui->checkBoxUseBaseline,
                           HriEnv->getObject(HricsParam::ioc_use_baseline));
  new connectCheckBoxToEnv(m_ui->checkBoxSplitMotions,
                           HriEnv->getObject(HricsParam::ioc_split_motions));
  new connectCheckBoxToEnv(m_ui->checkBoxTrainingDataset,
                           HriEnv->getObject(HricsParam::ioc_training_dataset));
  new connectCheckBoxToEnv(
      m_ui->checkBoxConservativeBaseline,
      HriEnv->getObject(HricsParam::ioc_conservative_baseline));

  new connectCheckBoxToEnv(m_ui->checkBoxParallelJob,
                           HriEnv->getObject(HricsParam::ioc_parallel_job));
  new connectCheckBoxToEnv(m_ui->checkBoxShowReplanning,
                           HriEnv->getObject(HricsParam::ioc_show_replanning));

  connect(m_ui->pushButtonRunIoc, SIGNAL(clicked()), this, SLOT(runIoc()));

  new connectComboBoxToEnv(m_ui->comboBoxIoc,
                           HriEnv->getObject(HricsParam::ioc_phase));
  new connectComboBoxToEnv(m_ui->comboBoxPlannerType,
                           HriEnv->getObject(HricsParam::ioc_planner_type));
  new connectComboBoxToEnv(m_ui->comboBoxIK,
  ...
  new SpinBoxConnector(this,
                       ...
                       HriEnv->getObject(HricsParam::ioc_spheres_power));
  new SpinBoxConnector(this,
                       m_ui->spinBoxSampleIteration,
                       HriEnv->getObject(HricsParam::ioc_sample_iteration));
  new SpinBoxConnector(this,
                       m_ui->spinBoxNbOfWayPoints,
                       HriEnv->getObject(HricsParam::ioc_nb_of_way_points));

  new SpinBoxConnector(this,
                       m_ui->doubleSpinBoxStandardDev,
                       HriEnv->getObject(HricsParam::ioc_sample_std_dev));
  new SpinBoxConnector(this,
                       m_ui->doubleSpinBoxStandardDevIK,
                       HriEnv->getObject(HricsParam::ioc_sample_std_dev_ik));

  new SpinBoxConnector(this,
                       m_ui->spinBoxSphereLayerToDraw,
                       HriEnv->getObject(HricsParam::ioc_spheres_to_draw));
  connect(m_ui->spinBoxSphereLayerToDraw,
          SIGNAL(valueChanged(int)),
          m_mainWindow,
...
void HriGestureWidget::setSplitCombobox(int id) {
  if (global_human_traj_simulator == NULL) return;
...
void HriGestureWidget::initSplitCombobox() {
  ...
  std::vector<std::string> splits_full =
      global_human_traj_simulator->getMotionsNames();
...
void HriGestureWidget::initLegibleCost() {
  new connectCheckBoxToEnv(m_ui->checkBoxUseLegibleCost,
                           PlanEnv->getObject(PlanParam::useLegibleCost));
}
...
void HriGestureWidget::initClassification() {
  connect(m_ui->pushButtonClassifyBad,
          SIGNAL(clicked()),
          this,
          SLOT(classifyBad()));
  connect(m_ui->pushButtonClassifyGood,
          SIGNAL(clicked()),
          this,
          SLOT(classifyGood()));
  connect(m_ui->pushButtonClassifyBothBad,
          SIGNAL(clicked()),
          this,
          SLOT(classifyBothBad()));
  connect(m_ui->pushButtonClassifyInit,
          SIGNAL(clicked()),
          this,
          SLOT(classifyInit()));
}
...
void HriGestureWidget::classifyGood() { cout << "classify good" << endl; }

void HriGestureWidget::classifyBad() { cout << "classify bad" << endl; }
