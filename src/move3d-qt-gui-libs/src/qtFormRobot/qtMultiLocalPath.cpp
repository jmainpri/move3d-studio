...
MultiLocalPathWidget::MultiLocalPathWidget(QWidget *parent, p3d_rob* rob)
{
  m_currentRobot = rob;
...
{
  p3d_rob *r = m_currentRobot; /* current robot */
...
void MultiLocalPathWidget::createMultiLocalPathList_obj() 
{
	...
	for(int i=0; i<r->mlp->nblpGp; i++) 
	{
		...
		m_MLP_Validator.push_back( new GroupValidator(0,m_currentRobot) );
...
//------------------------------------------------------------------------
GroupValidator::GroupValidator(QWidget *parent,p3d_rob* rob)
{
	m_currentRobot = rob;
}
